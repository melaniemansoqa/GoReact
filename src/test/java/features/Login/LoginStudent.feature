...
Feature: Student Login
  ...
  Scenario Outline: 02. Validate that a student cannot login to web app when using invalid password
    Given a user is on the base page
    ...
    Examples:
      | email | password |
      | themelaniemanso+student@gmail.com | wrongpassword |

  Scenario Outline: 03. Validate that a student can login to web app
    Given a user is on the base page
    When user inputs "<email>" on email field
    And user inputs "<password>" on password field
    And user clicks Login button
    ...
    Examples:
      | email | password |
      | themelaniemanso+student@gmail.com | Password**123 |
